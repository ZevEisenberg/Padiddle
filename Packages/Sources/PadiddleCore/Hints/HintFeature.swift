  ...
  var body: some FeatureProtocol<State, Action> {
    Update { state, action in
      ...
      case .isRecordingChanged(let isRecording):
        ...
        store.addTask { waitToShowRecordPrompt.cancel() }
        ...
        if isRecording {
          ...
          store.addTask { waitToShowSpinPrompt.cancel() }
        }
    ...
    .onMount { _ in
      // Observing `isRecording` is scoped to the feature's lifetime, not to the reminder, so that
      // re-triggering `start` can't stack up duplicate observation loops.
      @SharedReader(.isRecording) var isRecording
    ...
    .onTrigger(store.start) { state in
      ...
      guard state.hintState == .waitToShowRecordPrompt else {
        return
      }

      // Restart the countdown from zero each time we're asked to remind the user.
      store.addTask { waitToShowRecordPrompt.cancel() }
