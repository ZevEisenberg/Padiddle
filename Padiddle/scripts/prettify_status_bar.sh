...
if [ $(version "${TARGET_DEVICE_OS_VERSION}") -ge $(version "13") ]; then
    ...
    xcrun simctl status_bar "${TARGET_DEVICE_IDENTIFIER}" override --time "${timeString}" --dataNetwork wifi --wifiMode active --wifiBars 3 --cellularMode active --operatorName $(echo -e $zeroWidthSpace) --cellularBars 4 --batteryState discharging --batteryLevel 100
fi
