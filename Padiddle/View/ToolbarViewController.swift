...
class ToolbarViewController: UIViewController {
  ...
  @available(*, unavailable) override required init(nibName _: String?, bundle _: Bundle?) {
...
extension ToolbarViewController {
  ...
  private func configureViews() {
    ...
    recordButtonBack.image = UIImage(resource: .recordButtonBack)
    view.addSubview(recordButtonBack)

    // Toolbar
    toolbarView.backgroundColor = UIColor(resource: .toolbar)
    toolbarBottom.backgroundColor = UIColor(resource: .toolbar)
    ...
    toolbarHairline.backgroundColor = UIColor(resource: .toolbarHairline)
    ...
    for spacerView in spacerViews {
      spacerView.backgroundColor = nil
    }
    ...
    recordButton.setImage(UIImage(resource: .recordButtonFront), for: .normal)
    ...
    for spacerView in spacerViews.dropFirst().dropLast() {
      spacerView.widthAnchor == 20
    }
...
extension ToolbarViewController {
  ...
  @objc func shareTapped() {
    ...
    let restoreShareButton: (UIViewController) -> Void = { presentedViewController in
      ...
      guard let popoverController = presentedViewController.popoverPresentationController else {
        return
      }
